...
//DODAWANIE UZYTKOWNIKOW
    ...
    bcrypt.hash(password, 10, (bcryptErr, hash) => {
      if (bcryptErr) {
        ...
        db.query(`INSERT INTO logowanie (login,haslo) VALUES ('${username}','${hash}')`, (err, result) => {
          if (err) {
            res.json({"blad":"Ten login już istnieje!"})
          }
        })
      }
    });
  ...
  app.delete('/api/uzytkownicy/delete', (req, res) => {
    const {elementId} = req.body;
    db.query(`DELETE FROM logowanie WHERE id = ? `,elementId,(err)=>{
...
app.post('/api/fiszki/add', (req, res) => {
  ...
  const wykonawcyArray = Array.isArray(wykonawca) ? wykonawca : [wykonawca];
  const wykonawcyString = wykonawcyArray.join(', ');

  db.query(query, [nazwa, opis, sala, wykonawcyString, deadline, tworca], (err, result) => {
    if (err) {
      console.error('Błąd zapisu do bazy danych: ', err);
      res.status(500).json({ error: 'Błąd zapisu do bazy danych' });
    } else {
...
app.get('/api/fiszki', (req, res) => {
  const query = `SELECT *, GROUP_CONCAT(wykonawca SEPARATOR ', ') AS wykonawcy FROM fiszka GROUP BY id`;
